...
func main() {
	...
	for second := 0; second <= maxSeconds; second++ {
		...
		// safetyFactor := calculateSafetyFactor(robots, maxX, maxY)
		// fmt.Println("safety factor this second", safetyFactor)
		printRobots(robots, maxX, maxY)
	}

	fmt.Println("final safety factor", calculateSafetyFactor(robots, maxX, maxY))
}
...
func calculateSafetyFactor(robots []*robot, maxX int, maxY int) int {
	q1 := 0
	q2 := 0
	q3 := 0
	q4 := 0
